<template>
  <div>
    ...
    <el-card>
      ...
      <el-steps :active="activeNumber - 0" finish-status="success" align-center>
      ...
      <el-form :model="addForm" status-icon :rules="addFormRules" ref="addFormRef" label-width="100px"
        label-position="top">
        <el-tabs :tab-position="'left'" v-model="activeNumber" :before-leave="beforeTabLeave" @tab-click="tabClick">
          ...
          <el-tab-pane label="商品图片" name="3">
            <el-upload :action="actionUrl" :on-preview="handlePreview" :on-remove="handleRemove" list-type="picture"
              :headers="handerObj" :on-success="headerSuccess">
              <el-button size="small" type="primary">点击上传</el-button>
            </el-upload>
          </el-tab-pane>
          <el-tab-pane label="商品内容" name="4">
            <quill-editor v-model="addForm.goods_introduct">
            </quill-editor>
    ...
    methods: {
      ...
      async tabClick() {
        if (this.activeNumber === '1') {
          const {
            data: res
          } = await this.$http.get(`categories/${this.cat_id}/attributes`, {
            params: {
              sel: 'many'
            }
          })
